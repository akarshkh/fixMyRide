...
// Export Reports Route (require authentication and specific roles)
app.post("/api/reports/export", authenticateToken, requireRole(["admin", "manager"]), async (req, res) => {
  try {
    console.log("📊 Export request received from user:", req.user.username)
    const { period, dashboardStats, monthlyData } = req.body
    
    // Generate CSV content
    let csvContent = "Fix My Ride - CRM Analytics Report\n\n"
    
    // Add report metadata
    csvContent += `Report Period: ${period || 'monthly'}\n`
    csvContent += `Generated On: ${new Date().toLocaleString('en-IN')}\n`
    csvContent += `Generated By: ${req.user.name} (${req.user.username})\n\n`
    
    // Add dashboard statistics
    csvContent += "=== DASHBOARD STATISTICS ===\n"
    csvContent += `Total Customers: ${dashboardStats?.totalCustomers || 0}\n`
    csvContent += `Active Requests: ${dashboardStats?.activeRequests || 0}\n`
    csvContent += `Completed Requests: ${dashboardStats?.completedRequests || 0}\n`
    csvContent += `Total Revenue: ₹${(dashboardStats?.totalRevenue || 0).toLocaleString('en-IN')}\n\n`
    
    // Add monthly data if available
    if (monthlyData && monthlyData.length > 0) {
      csvContent += "=== MONTHLY REVENUE BREAKDOWN ===\n"
      csvContent += "Month,Revenue (₹),Services Completed\n"
      monthlyData.forEach(item => {
        csvContent += `${item.month},${item.revenue.toLocaleString('en-IN')},${item.services || 0}\n`
      })
      csvContent += "\n"
    }
    ...
    try {
      ...
      const topServices = await ServiceRequest.aggregate([
        { $match: { status: "Completed" } },
        { $group: { _id: "$issue", count: { $sum: 1 }, revenue: { $sum: "$cost" } } },
        { $sort: { count: -1 } },
        { $limit: 10 }
      ])
      ...
      if (topServices.length > 0) {
        ...
        csvContent += "Service Type,Count,Revenue (₹)\n"
        topServices.forEach(service => {
          csvContent += `"${service._id}",${service.count},${service.revenue.toLocaleString('en-IN')}\n`
        })
        csvContent += "\n"
      }
      ...
      const recentRequests = await ServiceRequest.find()
        ...
        .sort({ createdAt: -1 })
        .limit(20)
        .select("customerName phone vehicle issue status cost createdAt")
      
      if (recentRequests.length > 0) {
        csvContent += "=== RECENT SERVICE REQUESTS ===\n"
        csvContent += "Date,Customer,Phone,Vehicle,Issue,Status,Cost (₹)\n"
        recentRequests.forEach(request => {
          const customerName = request.customerName || (request.customerId ? request.customerId.name : "Unknown")
          const phone = request.phone || (request.customerId ? request.customerId.phone : "")
          const date = new Date(request.createdAt).toLocaleDateString('en-IN')
          csvContent += `${date},"${customerName}","${phone}","${request.vehicle}","${request.issue}",${request.status},${request.cost.toLocaleString('en-IN')}\n`
        })
      }
      
    } catch (analyticsError) {
      console.error("❌ Error fetching additional analytics:", analyticsError)
      csvContent += "Note: Some analytics data could not be retrieved\n"
    }
    
    // Set headers for CSV download
    res.setHeader('Content-Type', 'text/csv; charset=utf-8')
    res.setHeader('Content-Disposition', `attachment; filename="crm-report-${period}-${new Date().toISOString().split('T')[0]}.csv"`)
